...
func TestAutoStrategy_AddRemoveInbox(t *testing.T) {
	...
	// Verify tracked internally
	s.mu.RLock()
	found := false
	for _, i := range s.inboxes {
		if i.Hash == inbox.Hash {
			found = true
			break
		}
	}
	...
	if !found {
		t.Error("inbox was not added to tracked inboxes")
	}
	...
	if found {
		t.Error("inbox was not removed from tracked inboxes")
	}
}
...
func TestAutoStrategy_Start(t *testing.T) {
	...
	// Verify inboxes were tracked
	s.mu.RLock()
	if len(s.inboxes) != 2 {
		t.Errorf("inboxes count = %d, want 2", len(s.inboxes))
	}
	s.mu.RUnlock()
